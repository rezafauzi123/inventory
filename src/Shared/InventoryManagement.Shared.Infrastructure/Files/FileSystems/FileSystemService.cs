...
namespace InventoryManagement.Shared.Infrastructure.Files.FileSystems;
...
public class FileSystemService : IFileService
{
    ...
    public Task<bool> IsFileExistAsync(string fileName, CancellationToken cancellationToken)
    {
        //20230103_test.txt
        var s = fileName.Split('_');
        if (s.Length != 2)
        ...
        return Task.FromResult(!File.Exists(fullPath) ? false : true);
    }
